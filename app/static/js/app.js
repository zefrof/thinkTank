...
function suggestCards(cards) {
	...
	$.ajax({
		...
		success: function(response) {
			...
			if(cards.length == 0) {
				target.innerText = "No results found. Please try reducing the number of cards, or use less restricting filters.";
			}
			
			for(var i = 0; i < cards.length; i++) {
				var div = document.createElement('div');
				div.classList = "cell small-12 medium-4 large-3 tt-card-div";
				var img = document.createElement('img');
				img.classList = "tt-image";
				img.src = cards[i][2];
				div.appendChild(img);
				var label = document.createElement('label');
				label.classList = "tt-card-label";
				label.innerText = cards[i][0] + " $" + cards[i][1];
		...
		error: function(response) {
			target.innerHTML = "";
			target.innerText = "There was an error";
		}
	});
}